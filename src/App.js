...
// Este código es para un proyecto de React desplegado (ej. en GitHub Pages)
// Asume que las variables de entorno están en un archivo .env en la raíz del proyecto.
...
function Tasks({ db, groupId }) {
    ...
    useEffect(() => {
        if (!groupId) return;
        setLoading(true);
        // CAMBIO: Ahora la ruta usa un groupId compartido en lugar de un userId privado.
    ...
    const handleToggleTask = async (task) => {
        const taskDocRef = doc(db, `groups/${groupId}/tasks`, task.id);
                ...
                <ul className="space-y-3">
                    {tasks.map((task) => (
                        <li key={task.id} className={`flex items-center justify-between p-3 rounded-lg transition ${task.completed ? 'bg-green-100 text-gray-500' : 'bg-gray-100'}`}>
                            <span className={`flex-grow cursor-pointer ${task.completed ? 'line-through' : ''}`} onClick={() => handleToggleTask(task)}>
                                {task.text}
                            </span>
                            <button onClick={() => handleDeleteTask(task.id)} className="ml-4 text-red-500 hover:text-red-700 transition"><TrashIcon className="w-5 h-5" /></button>
                        </li>
                    ))}
                </ul>
            )}
...
function MarketList({ db, groupId }) {
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(q, (querySnapshot) => {
            ...
            itemsData.sort((a, b) => {
                 if (a.checked === b.checked) { return (b.createdAt?.seconds || 0) - (a.createdAt?.seconds || 0); }
    ...
    const handleAddItem = async (e) => {
        ...
        const marketCollectionPath = `groups/${groupId}/market_items`;
        await addDoc(collection(db, marketCollectionPath), { name: newItem, checked: false, createdAt: serverTimestamp() });
        setNewItem('');
    };

    const handleToggleItem = async (item) => {
        const itemDocRef = doc(db, `groups/${groupId}/market_items`, item.id);
        await updateDoc(itemDocRef, { checked: !item.checked });
    };

    const handleDeleteItem = async (itemId) => {
        const itemDocRef = doc(db, `groups/${groupId}/market_items`, itemId);
                ...
                <ul className="space-y-3">
                    {items.map((item) => (
                        <li key={item.id} className={`flex items-center justify-between p-3 rounded-lg transition ${item.checked ? 'bg-green-100 text-gray-500' : 'bg-gray-100'}`}>
                           <span className={`flex-grow cursor-pointer ${item.checked ? 'line-through' : ''}`} onClick={() => handleToggleItem(item)}>{item.name}</span>
                            <button onClick={() => handleDeleteItem(item.id)} className="ml-4 text-red-500 hover:text-red-700 transition"><TrashIcon className="w-5 h-5" /></button>
                        </li>
                    ))}
                </ul>
            )}
...
function Expenses({ db, groupId }) {
    ...
    const handleAddExpense = async (e) => {
        ...
        const expensesCollectionPath = `groups/${groupId}/expenses`;
        await addDoc(collection(db, expensesCollectionPath), { description, amount: expenseAmount, createdAt: serverTimestamp() });
        setDescription('');
        setAmount('');
    };
                            ...
                            {expenses.map((expense) => (
                                <li key={expense.id} className="flex items-center justify-between p-3 rounded-lg bg-gray-100">
                                    ...
                                    <div className="flex items-center">
                                        ...
                                        <button onClick={() => handleDeleteExpense(expense.id)} className="ml-4 text-red-500 hover:text-red-700 transition"><TrashIcon className="w-5 h-5" /></button>
                                    </div>
                                </li>
                            ))}
                        </ul>
...
export default function App() {
    ...
    // Efecto para gestionar la autenticación anónima
    useEffect(() => {
        const auth = getAuth();
    ...
    // Efecto para gestionar el ID del grupo desde la URL
    useEffect(() => {
        ...
        if (initialGroupId) {
            ...
            // Genera un ID corto y aleatorio si no hay uno en la URL
    ...
    const copyUrlToClipboard = () => {
        navigator.clipboard.writeText(window.location.href).then(() => {
            setCopied(true);
            setTimeout(() => setCopied(false), 2000); // El mensaje de "copiado" desaparece después de 2 segundos
        });
    }

    const renderView = () => {
        if (!isAuthReady || !groupId) {
            return <div className="text-center p-10"><p className="text-lg">Inicializando y conectando grupo...</p></div>;
        }
        // Pasamos el groupId a los componentes hijos
        switch (view) {
    ...
    const NavButton = ({ activeView, targetView, setView, children, icon }) => {
        ...
        return (
             <button onClick={() => setView(targetView)} className={`flex-1 flex flex-col sm:flex-row items-center justify-center gap-2 p-3 font-semibold rounded-t-lg transition-all duration-300 ${isActive ? 'bg-white text-blue-600 shadow-md' : 'bg-gray-200 text-gray-600 hover:bg-gray-300'}`}>
                 {icon}
                 ...
                 <footer className="text-center mt-8 text-sm text-gray-500 bg-blue-50 border border-blue-200 p-4 rounded-lg">
                    ...
                    <div className="flex justify-center mt-2">
                        <button onClick={copyUrlToClipboard} className="flex items-center gap-2 bg-blue-500 text-white font-bold py-2 px-4 rounded-lg hover:bg-blue-600 transition duration-300 shadow">
                           <CopyIcon />
                           {copied ? '¡URL Copiada!' : 'Copiar enlace de invitación'}
                        </button>
                    </div>
                </footer>
            </div>
